...
- [x] **Адаптивный UI** — подстройка под любой размер экрана (Termux 40×20 — ПК 160×50)
  ...
  - BoardCell и HandCard имеют ОДИНАКОВЫЙ размер (cell=hand)

**TODO (в работе):**

- [ ] **🎨 Косметика руки** — сделать руку визуально привлекательной
  - Тень под картами (символы `▀▄▌▐` или цветовая подсветка снизу)
  - Иконка масти/типа карты слева (▲ ▼ ☘ ⚔ ❀ — гриб/меч/цветок/...)
  - Рамка вокруг выбранной карты (двойная линия, ярче)
  - Цветовая индикация стоимости (c:1-5) разными оттенками
  - Эффект "пружинки" при выборе (анимация появления)
  - Градиент фона карты в зависимости от редкости
  - Карта-подсказка: при наведении показывает полное описание в попапе

- [ ] **🖼 Графика и арт**
  - **Фоны уровней** — ASCII-арт заднего плана для каждого уровня (лес, пещера, болото, ...)
  - **Арт на картах** — каждая карта имеет свою иконку/символ (грибы разные: 🍄 ☘ ❀, враги: 👹 🐛 🕷)
  - **Портреты врагов** — ASCII-арт для каждого уровня (Лесной страж, Грибной колдун, ...)
  - **Иконки эффектов** — ⚔ ATK, ❤ HP, ✦ cost, ☠ poison, ❄ freeze и т.д.
  - **Анимация появления карты** — плавное "выпадение" с верха экрана
  - **Анимация смерти** — карта рассыпается на символы `*` `+` `·`
  - **Hover-эффект** — подсветка карты при наведении мыши

- [ ] **🎨 Темы и цветовые палитры**
  - 2-3 готовые темы: "Классическая" (зелёный грибной лес), "Тёмная" (мрачная пещера), "Яркая" (мультяшная)
  - Переключение тем через настройки
  - Возможность кастомной темы через JSON
...
- [ ] **🎯 Прочие улучшения UI**
  - Скролл карт в руке (если больше 4)
  - Drag-and-drop карт (опционально)
  - Tooltip с описанием карты при наведении
  - Анимация перехода между уровнями
  - Экран "О игре" с правилами
...
gantt
    ...
    Адаптивный UI (cell=hand)      :done, 1d
    🖼 Графика и арт                :3d
    🎨 Косметика руки              :2d
    Темы и палитры                 :2d
    ✨ Анимации движения и атак     :3d
    Эффекты и переходы             :2d
