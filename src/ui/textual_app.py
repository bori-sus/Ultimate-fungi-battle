...
class BoardCell(Widget):
    ...
    def _card_size_for(width: int, height: int = 0) -> tuple[int, int]:
        ...
        Ширина = как у СТАРЫХ горизонтальных карт.
        Высота = 1.5× от старой высоты (4 → 6), где влезает.
        Иначе высота подбирается чтобы вписаться в экран.

        Размеры:
            40×20:  7×4  — Termux портрет, ratio 1.75
            60×30:  10×5 — средний, ratio 2.0
            80×24:  14×4 — стандартный терминал, ratio 3.5
            100×40: 18×6 — 1.5× высоты, ratio 3.0
            120×40: 24×6 — широкий, ratio 4.0
        """
        # Старая горизонтальная ширина — единая для cell и hand
        if width < 50:
            old_w = 7
        elif width < 70:
            old_w = 10
        ...
        # Целевая высота 1.5× от старой (4 → 6)
        target_h = 6

        # Если экран низкий — уменьшаем до максимально возможной
        if height > 0:
            # main(4*h) + hand(h-1) + top(1) + touch(2) ~= 5h + 2
            max_h = max(3, (height - 2) // 5)
            target_h = min(target_h, max_h)
            target_h = max(3, target_h)

        return old_w, target_h
