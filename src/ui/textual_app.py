...
class BoardCell(Widget):
    ...
    def _card_size_for(width: int, height: int = 0) -> tuple[int, int]:
        ...
        ШИРОКИЕ горизонтальные карты (как раньше, как просил пользователь).
        Ширина — 5 клеток влезают в строку + запас.
        Высота — 4 ряда помещаются с top+touch+hand.

        Примеры:
            40×20:  7×3   — Termux портрет
            60×30:  10×5  — Termux ландшафт
            80×24:  13×4  — стандартный терминал
            100×40: 16×7  — широкие карты
            120×40: 20×7  — очень широкие
            130×50: 21×9  — макси
        """
        # Ширина: 5 шт + 1 запас на руку
        cell_w = max(7, width // 6)
        # Высота: 4 ряда влезают с top+touch+hand
        cell_h = max(3, (height - 3) // 5)
