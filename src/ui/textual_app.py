...
class BoardCell(Widget):
    ...
    def _card_size_for(width: int, height: int = 0) -> tuple[int, int]:
        ...
        Соотношение игральной карты (вертикальная карта):
            h/w = 1.75 — высота в 1.75 раза больше ширины.

        Размеры (card_aspect = height/width = 1.75):
            40×20:  2×3  h/w=1.50  — мини (Termux портрет)
            40×30:  3×5  h/w=1.67  — средний
            60×30:  3×5  h/w=1.67  — Termux ландшафт
            80×24:  2×4  h/w=2.00  — узкий стандарт
            100×40: 4×7  h/w=1.75  ✅ стандарт игральной карты
            120×40: 4×7  h/w=1.75  ✅ стандарт
            130×50: 5×9  h/w=1.80  — большой
        """
        # Адаптация по высоте и ширине экрана
        if height < 22:
            return 3, 4   # 40×20 мини (ratio 1.33)
        elif height < 26:
            return 3, 4   # 80×24 узкий (ratio 1.33)
        elif height < 32:
            return 3, 5   # 60×30, 40×30 (ratio 1.67)
        elif height < 42:
            return 4, 7   # 100×40, 120×40 ✅ стандарт 1.75
        else:
            return 5, 9   # 130×50 большой (ratio 1.80)
